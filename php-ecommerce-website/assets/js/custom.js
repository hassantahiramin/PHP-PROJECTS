(function($){
        ...
        if($('.header').length >0){
            ...
            if(width > 991){
             ...
             $('.make-appoint').click(function () {
                 var $this = $(this);
                 var pro_search_form = $this.parents('.pro_search').find('.pro_search-form');
                 if ($this.hasClass('open')) {
                     pro_search_form.slideDown(500);
                     $this.removeClass('open');
                 } else {
                     pro_search_form.slideUp(500);
                     $this.addClass('open');
                 }
             });
          ...
          /*---------------------------------------------------------------------------------- */
           if ( jQuery().autosize ) {
             $('#pro_search_form_two textarea').autosize();
             $('#pro_search_form_three textarea').autosize();
             $('#pro_search_form_main textarea').autosize();
           }
    /* ---------------------------------------------
    ...
    $(document).ready(function() {
        ...
        $(document).on('click','.button-add-to-cart',function(){
            ...
            var product_article = $("#product_article").val();
            var product_image = $("#product_image").val();
            var result ='';
            result += '<div class="popup-add-to-cart">';
            result +='       <div class="message">';
            result +='          <i class="fa fa-check-circle-o"></i>';
            result +='          <p>You Have Added' + product_name + 'To Shopping Cart</p>';
            result +='      </div>';
        ...
        */
        ...
        //Image product zoom
         $('.product-feture').each(function(){
          ...
          var CatalogRequestForm = $("#catalog_request_form");
          CatalogRequestForm.on('click','#btn-catalog-request', function(e) {
            e.preventDefault();
            var selected_catalog = $('#selected_catalog').val().replace(/<|>/g, ""),
            customer_name = $('#customer_name').val().replace(/<|>/g, ""),
            email = $('#email').val().replace(/<|>/g, ""),
            company = $('#company').val().replace(/<|>/g, ""),
            address = $('#address').val().replace(/<|>/g, ""),
            telephone = $('#telephone').val().replace(/<|>/g, ""),
            message = $('#message').val().replace(/<|>/g, "");
              ...
              $(this).html('Loading...');
              var t = $(this);
              $.post('/handlers/catalog_request.php',data,function(result){
                  if(result.trim()=="done"){
                      $('#selected_catalog').val('');
                      $('#customer_name').val('');
                      $('#email').val('');
                      $('#address').val('');
                      $('#company').val('');
                      $('#telephone').val('');
                      $('#message').val('');
                      $('#message-box-catalog').html('<div class="alert alert-info">Your message was sent successfully. Thanks</div>');
                  }else{
                      $('#message-box-catalog').html(result);
                  }
                  $('#catalog_request_form')[0].reset();
                  t.html('SEND MESSAGE');
              })
          })
        ...
        ContactForm.on('click','#btn-contact-form', function(e) {
          e.preventDefault();
          ...
          message = $('#message').val().replace(/<|>/g, "");
            ...
            $(this).html('Loading...');
            var t = $(this);
            $.post('/handlers/contact_form.php',data,function(result){
                if(result.trim()=="done"){
                    $('#customer_name').val('');
                    $('#email').val('');
                    $('#token').val('');
                    ...
                    $('#telephone').val('');
                    $('#message').val('');
                    $('#message-box-contact').html('<div class="alert alert-info">Your message was sent successfully. Thanks</div>');
                }else{
        ...
        /* Send Contact Form*/
        var NewsLetterForm = $("#news_letter_form");
        NewsLetterForm.on('click','#btn-newsletter', function(e) {
          e.preventDefault();
          var email_address = $('#email_address').val().replace(/<|>/g, "");
          var data = {
            email_address: email_address
          }
            $.post('/handlers/newsletter_form.php',data,function(result){
                if(result.trim()=="done"){
                    $('#email_address').val('');
                    $('#message-box-newsletter').html('<div class="alert alert-info">Your message was sent successfully. Thanks</div>');
                }else{
                    $('#message-box-newsletter').html(result);
                }
