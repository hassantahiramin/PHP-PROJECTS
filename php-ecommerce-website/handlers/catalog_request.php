<?php
  ...
  if(Input::exists()){
    ...
    $validation = $validate->check($_POST, array(
      'selected_catalog' => array('required' => true),
  ...
  if($validation->passed()){
    $selected_catalog = filter_input(INPUT_POST, 'selected_catalog', FILTER_SANITIZE_STRING);
    ...
    $customer_message = filter_input(INPUT_POST, 'message', FILTER_SANITIZE_STRING);
    $selected_catalog = clean($selected_catalog);
    ...
    $selected_catalog = $purifier->purify($selected_catalog);
    $customer_name    = $purifier->purify($customer_name);
    $email 	          = $purifier->purify($email);
    $company 	        = $purifier->purify($company);
    $address 	        = $purifier->purify($address);
    $telephone 	      = $purifier->purify($telephone);
    $customer_message = $purifier->purify($customer_message);
    $date             = date('Y');
    ...
    if ($captcha_success->success==false) {
      echo "<div class='alert alert-danger alert-dismissable fade in'>
              <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><b>You are not verified by Google reCAPTCHA</b>
             </div>";
    }
    else if ($captcha_success->success==true) {
    $subject    = "Catalog Request From $customer_name.";
                     ...
                     <table role='presentation' cellspacing='0' cellpadding='0' border='0' width='100%'>
                       <tr>
                         <td style='padding: 40px; font-family: sans-serif; font-size: 15px; line-height: 20px; color: #555555;'>
                           <h1 style='margin: 0 0 10px 0; font-family: sans-serif; font-size: 24px; line-height: 27px; color: #333333; font-weight: normal;'>Catalog Request Details.</h1>
                           <p style='margin: 0;'><strong>Selected Catalog:                </strong> $selected_catalog</p>
                           <p style='margin: 0;'><strong>Contact Person:                  </strong> $customer_name</p>
                           <p style='margin: 0;'><strong>Company Name:                    </strong> $company</p>
                           <p style='margin: 0;'><strong>Address:                         </strong> $address</p>
                           <p style='margin: 0;'><strong>Email:                           </strong> $email</p>
                           <p style='margin: 0;'><strong>Phone:                           </strong> $telephone</p>
                           <p style='margin: 0;'><strong>Customer Message:                </strong> $customer_message</p>
                    ...
                    <table role='presentation' cellspacing='0' cellpadding='0' border='0' align='center' width='100%' style='max-width: 680px;'>
                      <tr>
                        <td style='padding: 40px 10px;width: 100%;font-size: 12px; font-family: sans-serif; line-height:18px; text-align: center; color: #888888;' class='x-gmail-data-detectors'>
                          ...
                          <br><br>
                          This Catalog Request Email Was Generated With Business Management Systems Exporter's Edition.
                        </td>
                      </tr>
                    </table>
                    <!-- Email Footer : END -->
      ...
      try
        {
         ...
         $mail->addCC('hello@hassanamin.com', 'Hassan Amin');
         ...
         if($mail->Send())
         {
           ...
           <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><b>Your Request Catalog has been Successfully Submitted.</b></div>";
         }
        }
